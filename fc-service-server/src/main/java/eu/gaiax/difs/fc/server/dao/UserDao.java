...
public class UserDao {
  ...
  public static UserRepresentation toUserRepo(User user) {
    ...
    //   userRepo.setCredentials(Collections.singletonList(createPasswordCredentials
    //   (INITIAL_PASSWORD)));
  ...
  public static UserProfile toUserProfile(UserRepresentation userRepo) {
    ...
    return new UserProfile(participantId, userRepo.getFirstName(), userRepo.getLastName(),
        userRepo.getEmail(),
        userRepo.getRealmRoles(), userRepo.getId(),
        userRepo.getFirstName() + " " + userRepo.getLastName());
  }
  ...
  public UserProfile create(User user) {
    ...
    if (response.getStatus() != HttpStatus.SC_CREATED) {
      ...
      log.info("create.error; status {}:{}, {}", response.getStatus(), response.getStatusInfo(),
          message);
  ...
  public UserProfile delete(String userId) {
    ...
    if (response.getStatus() != HttpStatus.SC_NO_CONTENT) {
      String message = getErrorMessage(response);
      log.info("delete.error; status {}:{}, {}", response.getStatus(), response.getStatusInfo(),
          message);
      throw new ConflictException(message);
    }
